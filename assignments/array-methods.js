...
let fullNames = [];
  runners.forEach(function(item){fullNames.push(`${item.first_name} ${item.last_name},`)})
...
runnersLargeSizeShirt = runners.filter(z => z.shirt_size ==="L")
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let donations = runners.map(x =>x.donation)
let ticketPriceTotal = donations.reduce((acc,x)=> acc + x)
